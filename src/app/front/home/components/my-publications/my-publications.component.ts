...
export class MyPublicationsComponent implements OnInit{
  ...
  async findMessages(){
    const findMyMessages = httpsCallable(this.functions, 'findMyPots');
  ...
  async checkInput() {
    if (this.inputValue) {
      ...
      this.inSearch = true;
      const findMyMessages = httpsCallable(this.functions, 'findMyPots');
      this.myMessages = (await findMyMessages({ date: new Date(this.inputValue), userId: this.auth.currentUser!.uid})).data as Message[];
      this.index = 1;
      this.results = [];
      for (let i = 0; i < this.myMessages.length; i += 2) {
        this.results.push(this.myMessages.slice(i, i + 2));
